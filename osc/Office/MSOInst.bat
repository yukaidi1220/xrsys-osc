...
if %officevernum% GEQ 14 (
  ...
  if "%officever%"=="365" (
    ...
    if exist "%SystemDrive%\Windows\System32\wbem\WMIC.exe" (
      ...
      for /f "tokens=2 delims==" %%G in ('powershell -NoLogo -NoProfile -EncodedCommand ""') do (if defined SKUID (call set "SKUID=!SKUID! %%G") else (call set "SKUID=%%G"))
    )
    for %%# in (%SKUID%) do (
